...
/*
  ==========================================================
  RC → Arduino → BLDC controller (stabiliseret version)
  ----------------------------------------------------------
  - CH3 → D2 (INT0): Throttle (PWM)
  - CH4 → D4 (PCINT20): Direction (PWM)
  - D7  → FR: Retning (til controller)
  - D8  → EN: Enable
  - D9  → SV: PWM speed
  ==========================================================
*/
...
// Deadband omkring midtpunktet (for at ignorere jitter)
const int DEAD_BAND_US = 25;  // ±25 µs = ingen bevægelse
...
void setup() {
  ...
  PCICR |= (1 << PCIE2);   // enable PCINT on port D
  ...
  Serial.println(F("RC→BLDC stabilized (INT0+PCINT2). D2=CH3 throttle, D4=CH4 dir, D9 PWM."));
}
...
void loop() {
  ...
  noInterrupts();
  if (newPulseDir) {
    // Samme smoothing på retningssignal
    lastWidthDir = (lastWidthDir * 3 + pulseWidthDir) / 4;
    newPulseDir = false;
  }
  interrupts();
